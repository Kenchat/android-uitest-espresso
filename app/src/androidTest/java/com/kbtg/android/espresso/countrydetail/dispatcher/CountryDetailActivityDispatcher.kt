...
class CountryDetailActivityDispatcher : Dispatcher() {
    ...
    override fun dispatch(request: RecordedRequest): MockResponse {
        Log.d("LOG", "CountryDetailActivityDispatcher | url: ${request.requestUrl} | path: ${request.path}")
        return when (request.path) {
            ...
            API_DAYONE_COUNTRY_THAI -> {
                val mock = DispatcherUtil.getMockResponse(PathJsonConstants.API_DAYONE_COUNTRY)
                Log.d("LOG", "\t\t\t\t | ${PathJsonConstants.API_DAYONE_COUNTRY} | mock: ${mock}")
                mock
            }
            else -> {
